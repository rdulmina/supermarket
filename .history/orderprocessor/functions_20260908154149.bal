import ballerina/email;import ballerina/ftp;
...
@workflow:Activity
@display {label: "tcpReadBytes", iconPath: "https://bcentral-packageicons.azureedge.net/images/ballerina_tcp_1.13.6.png"}
isolated function tcpReadBytes() returns byte[] & readonly|error {
    byte[] & readonly result = check tcpClient->readBytes();
    return result;
}
...
isolated function emailSend(email:SmtpClient connection, string|string[] to, string subject, string from, string body) returns error? {
    check connection->send(to, subject, from , body          );
}
