...
function emailChangeRequest(workflow:Context ctx, OrderInfo input, OrderWorkflowData data) returns json|error {
    json jsonResult = check ctx->callActivity(httpGet, {connection: httpClient, path: "gh"});

    json result = check ctx->callActivity(activity:callRestAPI, {connection: httpClient, method: "GET", path: "/fdf"});
