...
function orderWorkflow(workflow:Context ctx, OrderInfo input, OrderWorkflowData data) returns json|error {
    anydata result = check ctx->callActivity(reserveInventory, {orderInfo: input}, retryPolicy = {maxRetries: 9});
    anydata paymentResult = check ctx->callActivity(chargePayment, {orderInfo: input});
    anydata emailResult = check ctx->callActivity(sendConfirmationEmail, {orderInfo: input});
